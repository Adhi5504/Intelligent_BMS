...
from risk_scoring import classify_confidence
import risk_scoring
...
RAW_LOG_PATH = os.path.join(DATA_DIR, "bms_telemetry_raw.csv")
FILTERED_LOG_PATH = os.path.join(DATA_DIR, "bms_telemetry_filtered.csv")
...
def log_to_csv(filepath, row, prediction=None):
    ...
    if prediction:
        # Check if it's the new 15-field risk_scoring output or raw ML prediction
        if "final_risk_score" in prediction:
            fault_str = prediction.get("predicted_fault", "Normal")
            confidence_val = prediction.get("prediction_confidence", 1.0)
            severity_val = prediction.get("fault_severity_score", 0.0)
            final_risk_val = prediction.get("final_risk_score", 0.0)
            override_val = prediction.get("safety_override", False)
            persistence = prediction.get("persistence_count", 0)
        else:
            fault_str = prediction.get("Fault Prediction", "Normal")
            confidence_val = prediction.get("known_class_confidence", 1.0)
        
    fault_map = {
        "Normal": 0, "Normal Operation": 0,
        "Cell Imbalance": 1, "Cell Imbalance Risk": 1,
        "Weak Cell": 2, "High Voltage Gradient": 2,
        "Overvoltage Risk": 3,
        "Undervoltage Risk": 4,
        "Overtemperature Risk": 5,
        "Overcurrent Risk": 6,
        "Sensor Fault": 7, "SENSOR_OR_DATA_INTEGRITY_FAULT": 7,
        "Unknown Fault / OOD": 8, "UNKNOWN_FAULT_OOD": 8
    }
    ...
    with open(filepath, mode="a", newline="", encoding="utf-8") as f:
        ...
        if not file_exists:
            writer.writerow([
                ...
                "prediction_confidence", "fault_severity", "final_risk", "safety_override", "persistence_count"
            ])
        writer.writerow([
            cycle, charge_discharge, timestamp, voltage, current, temperature, soc,
            cell_v[0], cell_v[1], cell_v[2], cell_v[3], cell_v[4], cell_v[5], cell_v[6], cell_v[7],
            round(delta_v, 4), driving_mode, fault_label,
            round(confidence_val, 3), round(severity_val, 3), round(final_risk_val, 3), override_val, persistence
        ])
...
def check_and_trigger_learning():
    ...
    with learning_lock:
        ...
        scratch_dir = os.path.join(OUTPUTS_DIR, "scratch")
        ...
        train_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "incremental_train.py")
...
# CHEMISTRY_CONFIGS imported from fault_risk_config
...
def get_active_profile_config():
    ...
    try:
        ...
        return {
            ...
            "cell_min_voltage": cell_params.get("minimum_voltage", 2.8),
            "cell_max_voltage": cell_params.get("maximum_voltage", 4.2),
            "cell_critical_min": cell_params.get("minimum_voltage", 2.8) - 0.1,
            "cell_critical_max": cell_params.get("maximum_voltage", 4.2) + 0.05,
            "temp_warn_limit": thresholds.get("overtemperature_warning_min", 45.0),
            "temp_critical_limit": thresholds.get("overtemperature_critical", 55.0),
            "current_warn_limit": thresholds.get("overcurrent_warning", 15.0),
            "current_critical_limit": thresholds.get("overcurrent_critical", 20.0),
            "imbalance_warn_limit": thresholds.get("imbalance_warning", 0.08),
            "imbalance_critical_limit": thresholds.get("imbalance_critical", 0.15),
            "is_fallback": False
        }
    except Exception as e:
        return None
...
# Legacy get_physics_informed_prediction replaced by risk_scoring.process_telemetry_risk
    ...
    with alert_lock:
        ...
        for old_a in active_alerts:
            if old_a["type"] not in new_keys:
                ...
                t = old_a["type"]
                if t == "Stress Predictor Output above Limit":
                    resolved_alert["resolution_reason"] = "Stress score dropped below limit."
                elif t == "High Deviation from Cycle History":
                    resolved_alert["resolution_reason"] = "Deviation normalized or cycle ended."
                elif t == "OOD Anomaly Detected":
                    resolved_alert["resolution_reason"] = "Signal returned to known distribution."
                elif t == "Temperature Spread Warning":
                    resolved_alert["resolution_reason"] = "Cell temperatures rebalanced."
                elif "Voltage" in t or "Current" in t:
                    resolved_alert["resolution_reason"] = "Parameter returned to safe range."
                else:
                    resolved_alert["resolution_reason"] = "Condition returned to safe baseline."
...
def run_alerts_evaluation():
    ...
    if not bt_connected:
        ...
        with telemetry_lock:
            buf_copy = list(telemetry_buffer)
        if buf_copy:
            latest_row = buf_copy[-1]
            prediction = None
            if len(buf_copy) == BUFFER_MAX_SIZE:
                try:
                    df_60 = pd.DataFrame(buf_copy)
...
CYCLE_HISTORY_PATH = os.path.join(DATA_DIR, "cycle_history.json")
...
def classify_confidence(score):
    if score >= 0.8:
        return {"score": score, "label": "High Confidence"}
    elif score >= 0.5:
        return {"score": score, "label": "Medium Confidence"}
    else:
        return {"score": score, "label": "Low Confidence"}
...
def get_cycle_analytics_summary():
    ...
    expected_soc = 50.0
    elapsed_sec = 0.0
    if active_cycle["start_time_epoch"] is not None:
    ...
    confidence_score = classify_confidence(0.10)
    ...
    if active_type == "Charge" and active_cycle["rows"]:
        ...
        if latest_raw_row:
            ...
            if active_cycle["rows"]:
                start_t = float(active_cycle["rows"][0].get("temperature", t))
            cell_v = [float(latest_raw_row.get(f"cell_v{i}", 3.2)) for i in range(1, 9)]
            spread = max(cell_v) - min(cell_v) if cell_v else 0.0
            
        unusual_drift = "None"
        temp_drift = t - start_t
        temp_stable = abs(temp_drift) < 2.0
        
        if not temp_stable:
            deviation_level = "Severe thermal anomaly"
            state_analysis_msg = f"Idle Analysis: Sudden temperature rise ({temp_drift:.1f}°C) detected while battery is at rest!"
            idle_status_msg = f"Abnormal thermal behavior detected. Possible external heat source or internal short."
            likely_fault_to_occur = "Overtemperature Risk"
            unusual_drift = "Temperature drift detected"
        elif spread > 0.12:
            deviation_level = "Possible weak cell behavior"
        ...
        idle_analysis = {
            ...
            "voltage_stable": spread <= 0.12,
            "current_stable": True,
            "soc_stable": True,
            "temp_stable": temp_stable,
            "unusual_drift": unusual_drift,
            "voltage_drift": f"{spread:.3f} V",
            "temp_drift": f"{temp_drift:.1f} °C",
            "soc_drift": "0.0 %",
            "msg": idle_status_msg
        }

    # Final degradation trend formatting
    if behavior_trend == "Degrading" or "Possible degradation" in deviation_level:
        behavior_trend = "Degrading"
        degradation_trend = "Possible degradation trend detected based on live and previous cycle comparison."
    elif len(history) < 3:
        degradation_trend = "Cycle history insufficient - monitoring only."
    else:
        degradation_trend = "Battery performance stable across active and completed cycles."
        behavior_trend = "Stable"
        
    # Make cycle engine confidence scale dynamically with active cycle progression
    cycle_progress = 0.1
    if active_type != "Idle" and elapsed_sec > 0:
        avg_duration = 3600.0  # Safe default
        if active_type == "Charge" and len(past_charges) > 0:
            avg_duration = sum(c.get("duration_sec", 3600) for c in past_charges) / len(past_charges)
        elif active_type == "Discharge" and len(past_discharges) > 0:
            avg_duration = sum(c.get("duration_sec", 3600) for c in past_discharges) / len(past_discharges)
        cycle_progress = min(1.0, elapsed_sec / avg_duration)
    ...
    # Dynamic confidence: starts low at the beginning of a cycle and grows as more active data is collected
    final_conf = max(0.10, base_conf * cycle_progress)
    confidence_score = classify_confidence(final_conf)
    confidence_reason = f"Baseline conf: {base_conf*100:.0f}% | Active progression: {cycle_progress*100:.0f}%"

    current_soc_raw = active_cycle["rows"][-1].get("soc") if active_cycle["rows"] else expected_soc
    safe_current_soc = float(current_soc_raw) if current_soc_raw is not None else float(expected_soc)
    safe_start_soc = float(active_cycle["start_soc"]) if active_cycle.get("start_soc") is not None else 50.0
    ...
    return {
        ...
        "current_soc": round(safe_current_soc, 1),
        "expected_soc": round(expected_soc, 1),
        "active_cycle": {
            "type": active_type,
            "elapsed_sec": round(elapsed_sec, 1),
            "start_soc": round(safe_start_soc, 1),
            "current_soc": round(safe_current_soc, 1)
        },
        "history_summary": {
            "total_cycles_completed": len(history),
            "avg_charge_duration": round(sum(c.get("duration_sec", 0) for c in past_charges) / len(past_charges), 1) if past_charges else 0,
            "avg_discharge_duration": round(sum(c.get("duration_sec", 0) for c in past_discharges) / len(past_discharges), 1) if past_discharges else 0
        },
        "confidence_score": confidence_score,
        "confidence_reason": f"Baseline conf: {base_conf*100:.0f}% | Active cycle prog: {cycle_progress*100:.0f}%",
...
MODEL_DIR = str(BASE_DIR.parent / "models")
...
def add_telemetry():
    ...
    try:
        ...
        if not is_valid:
            ...
            with telemetry_lock:
                buf_len = len(telemetry_buffer)
                live_row_val = telemetry_buffer[-1] if telemetry_buffer else None
            return jsonify({
                "buffer_length": buf_len,
                "status": "Waiting for valid live data",
                "prediction": None,
                "live_prediction": None,
                "live_row": live_row_val,
        ...
        live_prediction = risk_scoring.process_telemetry_risk(smoothed_row, prediction, chem_type, time.time())

        response_data = {
            "buffer_length": len(buf_copy),
...
def get_current_status():
    ...
    try:
        ...
        with telemetry_lock:
            buf_copy = list(telemetry_buffer)
        if not is_stale and bluetooth_connected and len(buf_copy) == BUFFER_MAX_SIZE:
            df_60 = pd.DataFrame(buf_copy)
            prediction = run_inference(df_60, MODEL_DIR)
        ...
        live_prediction = None
        if buf_copy and not is_stale:
            latest_row = buf_copy[-1]
            chem_type = detect_chemistry(latest_row)
            live_prediction = risk_scoring.process_telemetry_risk(latest_row, prediction, chem_type, time.time())
        ...
        return jsonify({
            ...
            "live_row": buf_copy[-1] if (buf_copy and not is_stale) else None,
            "raw_row": latest_raw_row,
            "learning_status": get_learning_status(),
            "cycle_analytics": get_cycle_analytics_summary(),
            "bluetooth_connected": bluetooth_connected,
...
def upload_datasheet():
    ...
    if file and file.filename.lower().endswith('.pdf'):
        ...
        if params:
            ...
            import fault_risk_config
            fault_risk_config.CHEMISTRY_CONFIGS["Active_Profile"] = profile
            CHEMISTRY_CONFIGS["Active_Profile"] = profile
...
def reset_telemetry_buffer():
    ...
    return jsonify({"success": True, "message": "Telemetry buffers reset"})
...
@app.route('/')
def backend_root():
    dashboard_path = os.path.join(PROJECT_ROOT, 'live_dashboard_v3.html')
...
@app.route('/ml-health', methods=['GET', 'OPTIONS'])
def ml_health_check():
    if request.method == 'OPTIONS':
        return app.make_default_options_response()
    try:
        model_ready = getattr(app, 'model_ready', False)
        return jsonify({
            "service": "ml_backend",
            "status": "running",
            "model_loaded": model_ready,
            "scaler_loaded": model_ready,
            "telemetry_available": len(telemetry_buffer) > 0,
            "prediction_ready": model_ready and len(telemetry_buffer) == BUFFER_MAX_SIZE,
            "last_prediction_timestamp": datetime.now().isoformat()
        })
    except Exception as e:
        return jsonify({"service": "ml_backend", "status": "error", "error": str(e)}), 500
...
if __name__ == '__main__':
    ...
    try:
        ...
        app.model_ready = False
        print(f"[ML ERROR] {type(exc).__name__}: {exc}")
        import traceback
        traceback.print_exc()
        print("[BACKEND] Continuing with telemetry services.")
