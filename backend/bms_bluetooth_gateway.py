...
# ==============================================================================
class BMSGateway:
    def __init__(self, bms_type: str, mac_address: str, api_url: str, poll_interval: float, verbose: bool, dataset_path: str = None):
    ...
    def post_telemetry(self) -> bool:
        """POSTs current telemetry payload to the Flask backend."""
        try:
    ...
    async def run_mock(self):
        if self.dataset_path and os.path.exists(self.dataset_path):
            self.log(f"Starting in MOCK Mode. Streaming dataset: {self.dataset_path}")
            import csv
            with open(self.dataset_path, mode='r') as file:
                ...
                for row in reader:
                    try:
                        self.latest_data = {
                            "voltage": float(row.get("Pack_Voltage", 25.6)),
                            "current": float(row.get("Pack_Current", 0.0)),
                            "temperature": float(row.get("Max_Temperature", 25.0)),
                            "soc": float(row.get("SOC", 50.0)),
                            "delta_v": float(row.get("Cell_Voltage_Difference", 0.0))
                        }
                        for i in range(1, 9):
                            self.latest_data[f"cell_v{i}"] = float(row.get(f"Cell_{i}_Voltage", 3.2))
                        for n in range(1, 5):
                            self.latest_data[f"ntc{n}"] = float(row.get(f"NTC_{n}_Temperature", 25.0))
            ...
            self.log("Dataset streaming complete!")
            while True:
                await asyncio.sleep(1.0)
        else:
            self.log("Starting in MOCK Mode. Generating synthetic BMS data (No dataset found)...")
            import math
            t_counter = 0
            while True:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default=None,
                        help="Path to a dataset CSV to stream in MOCK mode")
